...
type BackendModel = { id: string; name: string; kind: 'chat' | 'embedding'; baseUrl: string; model: string; dimension?: number; enabled: boolean; isDefault: boolean; hasApiKey: boolean; webSearchMode?: 'disabled' | 'qwen' | 'openai' | 'custom'; extraBody?: Record<string, unknown> }
...
type BackendPipeline = { id: string; time: string; bot: string; conversation: string; content: string; eventMs: number; contextMs: number; retrieval: { status: Status; ms: number; hit: string }; model: { status: Status; ms?: number; name?: string }; delivery: { status: Status; ms?: number }; totalMs: number }
...
function mapModel(item: BackendModel): ModelProfile {
  return { id: item.id, name: item.name, kind: item.kind, provider: 'OpenAI Compatible', model: item.model, baseUrl: item.baseUrl, maskedKey: item.hasApiKey ? '••••••••••••' : '未配置', status: item.enabled ? 'online' : 'offline', latency: 0, dimension: item.dimension, webSearchMode: item.webSearchMode || 'disabled', extraBody: item.extraBody || {} }
}
...
export const api = {
  ...
  models: {
    ...
    save: (profile: Partial<ModelProfile>, apiKey = '') => request<{ id: string }>({ method: profile.id ? 'PUT' : 'POST', url: profile.id ? `/model-profiles/${profile.id}` : '/model-profiles', data: { name: profile.name, kind: profile.kind, baseUrl: profile.baseUrl, apiKey, model: profile.model, dimension: profile.dimension, enabled: profile.status !== 'offline', isDefault: false, webSearchMode: profile.kind === 'chat' ? profile.webSearchMode || 'disabled' : 'disabled', extraBody: profile.kind === 'chat' ? profile.extraBody || {} : {} } }, { id: profile.id || `model-${Date.now()}` }),
