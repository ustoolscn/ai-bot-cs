<template>
  <div class="content-page">
    ...
    <div class="system-grid">
      ...
      <section class="settings-panel"><h2>消息与上下文</h2><el-alert v-if="settingsError" :title="settingsError" type="error" :closable="false" show-icon class="settings-load-error" /><el-form label-position="top"><el-form-item label="默认上下文消息数"><el-input-number v-model="settings.defaultContextLimit" :min="1" :max="100" :disabled="!settingsLoaded" /><small class="form-tip">包含当前问题；设置为 1 时只回答当前问题。</small></el-form-item><el-form-item label="AI 请求超时"><el-input-number v-model="settings.aiRequestTimeoutSeconds" :min="10" :max="600" :disabled="!settingsLoaded" /><span class="unit-label">秒</span></el-form-item><el-form-item label="消息记录保留"><el-input-number v-model="settings.messageRetentionDays" :min="1" :max="3650" :disabled="!settingsLoaded" /><span class="unit-label">天</span></el-form-item><div class="settings-notice"><p>设置保存在 PostgreSQL 中。</p><p>AI 请求超时保存后立即生效；默认上下文仅用于新会话；消息保留策略约 1 分钟内执行。</p><small v-if="settings.updatedAt">最近更新：{{ formatUpdatedAt(settings.updatedAt) }}</small></div><el-button type="primary" :loading="saving" :disabled="!settingsLoaded || saving" @click="saveSettings">保存设置</el-button></el-form></section>
